...
def sanitise_fields(instance, exclude=[], error_on_change=[]):
    if hasattr(instance, "__dict__"):
        for i in instance.__dict__:
            ...
            if i not in exclude and (isinstance(instance.__dict__[i], dict)):
                instance.__dict__[i] = sanitise_fields(instance.__dict__[i])

            elif isinstance(instance.__dict__[i], list) and i not in exclude:
            ...
            elif isinstance(instance.__dict__[i], str) and i not in exclude:
                ...
                if isinstance(instance.__dict__[i], dict):
                    check = instance.__dict__[i]
                    instance.__dict__[i] = sanitise_fields(
                        instance.__dict__[i],
        ...
        for i in instance:
            ...
            # remove html tags for all string fields not in the exclude list
            if i not in exclude and (isinstance(instance[i], dict)):
                instance[i] = sanitise_fields(instance[i])

            elif isinstance(instance[i], list) and i not in exclude:
                ...
                if isinstance(instance[i], str) and i not in exclude:
                    check = instance[i]
                    ...
                    error_on_change_list = list(
                        map(
                            ...
                            sub_error_on_change_list,
                        )
                    )

                    if isinstance(instance[i], dict):
                        check = instance[i]
                        instance[i] = sanitise_fields(
                            instance[i],
                            exclude=exclude_list,
                            error_on_change=error_on_change_list,
                        )
...
def get_qs_denied_site(search_text=""):
    ...
    qs_on_proposal = (
        ...
        .filter(q_include_proposal)
        .exclude(q_exclude_proposal)
        .exclude(wkb_geometry_processed=None)
        .values(
...
def get_qs_suspended_site(search_text=""):
    ...
    # 2.3. Issue query
    qs_on_approval = (
        ApiarySiteOnApproval.objects.select_related(
...
def get_qs_proposal(draft_processed, proposal=None, search_text="", include_pure_draft_site=False):
    ...
    if include_pure_draft_site:
        ...
        q_exclude_proposal |= Q(site_status__in=(SITE_STATUS_DRAFT,)) & Q(
            making_payment=False
        )  # Exclude pure 'draft' site
    q_exclude_proposal |= Q(site_status__in=(SITE_STATUS_DISCARDED,))
    q_exclude_proposal |= Q(
        site_status__in=(SITE_STATUS_APPROVED,)
    )  # 'approved' site should be included in the approval as a 'current'
    q_exclude_proposal |= Q(
        apiary_site__in=ApiarySite.objects.filter(is_vacant=True)
    )  # Vacant sites are already picked up above.  We don't want to pick up them again here.
    ...
    if draft_processed == "draft":
        qs_on_proposal = (
            ...
            .filter(q_include_proposal)
            .exclude(q_exclude_proposal)
            .filter(wkb_geometry_processed=None)
            .exclude(proposal_apiary=proposal_apiary)
            .values(
                "wkb_geometry_draft",
                "apiary_site__id",
                "site_status",
...
def get_qs_approval():
    ...
    q_include_approval &= Q(
        id__in=(
            ApiarySite.objects.filter(latest_approval_link__isnull=False).values_list(
                "latest_approval_link__id", flat=True
            )
        )
    )  # Include only the intermediate objects which are on the ApiarySite.latest_proposal_links
...
def validate_buffer(wkb_geometry, apiary_sites_to_exclude=None):
    ...
    sites = qs_on_approval.exclude(apiary_site__in=apiary_sites_to_exclude).filter(
        Q(wkb_geometry__distance_lte=(wkb_geometry, Distance(m=RESTRICTED_RADIUS)))
    )
    if sites:
        raise site_too_close_error
...
def get_qs_vacant_site_for_export():
    ...
    # 1. Collect Proposal Link IDs (excluding NULLs)
    latest_proposal_ids = qs_vacant_site.filter(latest_proposal_link__isnull=False).values_list(
        "latest_proposal_link_id", flat=True
    )

    fallback_proposal_ids = qs_vacant_site.filter(
        latest_proposal_link__isnull=True,
        proposal_link_for_vacant__isnull=False,
    ).values_list("proposal_link_for_vacant_id", flat=True)

    # Combine both ID sets
    proposal_link_ids = set(latest_proposal_ids) | set(fallback_proposal_ids)

    # 2. Collect Approval Link IDs (excluding NULLs)
    approval_link_ids = qs_vacant_site.filter(approval_link_for_vacant__isnull=False).values_list(
        "approval_link_for_vacant_id", flat=True
    )

    # 3. Querysets with pre-joined relations
    qs_vacant_site_proposal = ApiarySiteOnProposal.objects.filter(id__in=proposal_link_ids).select_related(
        "apiary_site",
        "site_category_draft",
        "proposal_apiary__proposal",
    )

    qs_vacant_site_approval = ApiarySiteOnApproval.objects.filter(id__in=approval_link_ids).select_related(
        "apiary_site",
        "site_category",
        "approval",
    )
...
def get_qs_proposal_for_export():
    ...
    # 1. Non-null latest_proposal_link IDs
    latest_proposal_link_ids = ApiarySite.objects.filter(latest_proposal_link__isnull=False).values_list(
        "latest_proposal_link_id", flat=True
    )

    # 2. Consolidated excluded statuses
    excluded_statuses = [
        SITE_STATUS_DRAFT,
        SITE_STATUS_DISCARDED,
        SITE_STATUS_PENDING,
    ...
    # 3. Base Queryset with pre-fetched relations
    qs_on_proposal = (
        ApiarySiteOnProposal.objects.filter(id__in=latest_proposal_link_ids)
        .exclude(Q(apiary_site__is_vacant=True) | Q(site_status__in=excluded_statuses))
        .select_related(
            "apiary_site",
            "site_category_draft",
            "proposal_apiary__proposal",
            # Preload applicant relation if applicable on Proposal model, e.g.:
            # "proposal_apiary__proposal__applicant",
            # "proposal_apiary__proposal__org_applicant",
        )
        .order_by("apiary_site_id", "-id")
        .distinct("apiary_site_id")
    )

    # 4. Split processed and draft geometries using Django isnull lookups
    qs_on_proposal_processed = qs_on_proposal.filter(wkb_geometry_processed__isnull=False)
    qs_on_proposal_draft = qs_on_proposal.filter(wkb_geometry_processed__isnull=True)
...
def get_qs_approval_for_export():
    ...
    # 1. Get non-null latest_approval_link IDs
    latest_link_ids = ApiarySite.objects.filter(latest_approval_link__isnull=False).values_list(
        "latest_approval_link_id", flat=True
    )

    # 2. Get vacant sites
    qs_vacant_site = _get_vacant_apiary_site()

    # 3. Excluded statuses in a single IN clause
    excluded_statuses = [
        SITE_STATUS_TRANSFERRED,
        SITE_STATUS_DRAFT,
        SITE_STATUS_PENDING,
    ...
    # 4. Construct queryset with pre-joined relations
    qs_on_approval = (
        ApiarySiteOnApproval.objects.filter(id__in=latest_link_ids)
        .exclude(Q(apiary_site__in=qs_vacant_site) | Q(site_status__in=excluded_statuses))
        .select_related(
            "apiary_site",
            "site_category",
            "approval",
            # Preload applicant relations if applicable on your Approval model, e.g.:
            # "approval__applicant",
...
def getProposalExport(filters, num):
    from disturbance.components.proposals.models import ApplicationType, Proposal
    ...
    columns = list(
        map(
            lambda proposal: (
                ...
                proposal[3]
                if proposal[3]
                else user_map.get(proposal[4])
                if user_map.get(proposal[4])
                else user_map.get(proposal[2]),
    ...
    columns = list(
        map(
            lambda compliance: (
                ...
                compliance[3]
                if compliance[3]
                else user_map.get(compliance[4])
                if user_map.get(compliance[4])
                else user_map.get(compliance[5]),
                compliance[6],
...
def getSiteExportFields(data):
    header = [
        ...
        "Latest Approval Link",
        "Is Vacant?",
        "Exempt from Radius Restriction?",
        "Longitude",
        "Latitude",
    ]
