...
class ProposalHistorySerializer(serializers.ModelSerializer):
    ...
    def get_versions(self, obj):
        entry_versions = VersionSerializer(Version.objects.get_for_object(obj), many=True)
...
class ApiaryChecklistQuestionSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ("id", "text", "answer_type", "checklist_type", "order")
...
class ApiaryChecklistAnswerSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ("id", "question", "answer", "proposal_id", "apiary_referral_id", "text_answer", "apiary_site_id")


class ApplicantAddressSerializer(serializers.ModelSerializer):
    class Meta:
        model = Address
        fields = ("id", "line1", "line2", "line3", "locality", "state", "country", "postcode")
    ...
    def validate(self, data):
        ...
        if not self.partial:
            if not data["period_from"]:
                field_errors["Period from"] = [
                    "Please select a date.",
                ]
            if not data["period_to"]:
                field_errors["Period to"] = [
                    "Please select a date.",
                ]
...
class ApiarySiteOnProposalProcessedGeometryExportSerializer(ApiarySiteOnProposalProcessedGeometrySerializer):
    ...
    def _get_proposal_data(self, relation):
        """
        Extracts and caches all required strings once per proposal.
        """
        prop_apiary = getattr(relation, "proposal_apiary", None)
        proposal = getattr(prop_apiary, "proposal", None)
        if not proposal:
            return {
                "lodgement_num": "",
                "org_name": "",
        ...
        prop_id = proposal.id
        if prop_id in self._proposal_cache:
            return self._proposal_cache[prop_id]

        lodgement_num = getattr(proposal, "lodgement_number", "") or ""

        # Check cross-proposal applicant cache key
        app_key = (
            getattr(proposal, "org_applicant_id", None)
            or getattr(proposal, "proxy_applicant_id", None)
            or getattr(proposal, "applicant_id", None)
            or prop_id
        )
        ...
        if app_key in self._applicant_cache:
            ...
            self._proposal_cache[prop_id] = app_data
            return app_data

        # Initial fetch of applicant
        try:
            applicant = getattr(proposal, "relevant_applicant", None)
        except Exception as e:
            logger.warning(f"[Proposal {lodgement_num}] Failed fetching EmailUserRO/Organisation: {e}")
            applicant = None

        if not applicant:
            logger.warning(
                f"[Proposal {lodgement_num}] (ID: {prop_id}) has no valid applicant / EmailUserRO not found."
            )
        ...
        # Address
        try:
            addr = proposal.relevant_applicant_address
        ...
        # Email
        try:
            email_str = proposal.relevant_applicant_email or ""
        except Exception:
            email_str = ""
        ...
        self._proposal_cache[prop_id] = data
        return data

    def get_proposal_lodgement_number(self, relation):
        return self._get_proposal_data(relation)["lodgement_num"]

    def get_approval_lodgement_number(self, relation):
        return ""

    def get_organisation_name(self, relation):
        return self._get_proposal_data(relation)["org_name"]

    def get_status(self, relation):
        return get_status_for_export(relation)

    def get_category(self, relation):
        cat = getattr(relation, "site_category_draft", None)
        return cat.get("name", "") if isinstance(cat, dict) else getattr(cat, "name", "")

    def get_surname(self, relation):
        return self._get_proposal_data(relation)["last_name"]

    def get_first_name(self, relation):
        return self._get_proposal_data(relation)["first_name"]

    def get_address(self, relation):
        return self._get_proposal_data(relation)["address"]

    def get_telephone(self, relation):
        return self._get_proposal_data(relation)["phone"]

    def get_mobile(self, relation):
        return self._get_proposal_data(relation)["mobile"]

    def get_email(self, relation):
        return self._get_proposal_data(relation)["email"]
        ...
        if check_coastline:
            ...
            if not feature:
                raise serializers.ValidationError(
                    [
                        f"Apiary Site: {self.instance.apiary_site.id} (lat: {wkb_geometry.coords[1]}, lng: {wkb_geometry.coords[0]}) is out of bounds."
                    ]
                )

        apiary_sites_to_exclude = (
            [
                self.instance.apiary_site,
            ]
            if self.instance.apiary_site
            else None
        )
...
class ProposalApiarySerializer(serializers.ModelSerializer):
    ...
    def get_site_remainders(self, proposal_apiary):
        ...
        for category in SiteCategory.CATEGORY_CHOICES:
            try:
                ...
                filter_site_fee_type = Q(
                    apiary_site_fee_type=ApiarySiteFeeType.objects.get(name=ApiarySiteFeeType.FEE_TYPE_RENEWAL)
                )
                ...
                fee_renewal = site_category.retrieve_fee_by_date_and_type(
                    today_local, ApiarySiteFeeType.FEE_TYPE_RENEWAL
                )
    ...
    def get_applicant_checklist_answers(self, obj):
        return ApiaryChecklistAnswerSerializer(
            obj.apiary_checklist.filter(question__checklist_role="applicant").order_by("question__order"), many=True
        ).data

    def get_assessor_checklist_answers(self, obj):
        return ApiaryChecklistAnswerSerializer(
            obj.apiary_checklist.filter(question__checklist_role="assessor")
            .filter(question__checklist_type="apiary")
    ...
    def get_assessor_checklist_answers_per_site(self, obj):
        answers = [
            a
            for a in self._load_apiary_checklist_cache(obj)
    ...
    def get_referrer_checklist_answers(self, obj):
        ...
        for referral in obj.proposal.referrals.all():
            answers = [
                a
                for a in checklist
                if a.apiary_referral_id == getattr(referral.apiary_referral, "id", None)
                and a.question.checklist_type == "apiary"
            ]
            answers.sort(key=lambda a: (getattr(a.question, "order", 0), getattr(a, "id", 0)))
        ...
        for referral in obj.proposal.referrals.all():
            for site in obj.apiary_sites.all():
                answers = [
                    ...
                    for a in checklist
                    if a.apiary_referral_id == getattr(referral.apiary_referral, "id", None)
                    and a.question.checklist_type == "apiary_per_site"
                    and getattr(a.apiary_site, "id", None) == getattr(site, "id", None)
                ]
                answers.sort(key=lambda a: (getattr(a.question, "order", 0), getattr(a, "id", 0)))
                qs = ApiaryChecklistAnswerSerializer(answers, many=True).data
                referral_list.append(
    ...
    def get_site_transfer_assessor_checklist_answers(self, obj):
        return ApiaryChecklistAnswerSerializer(
            obj.apiary_checklist.filter(question__checklist_role="assessor")
            .filter(question__checklist_type="site_transfer")
            .order_by("question__order"),
            many=True,
        ).data
        ...
        for referral in obj.proposal.referrals.all():
            for site in obj.get_relations():
                answers = [
                    ...
                    for a in checklist
                    if a.apiary_referral_id == getattr(referral.apiary_referral, "id", None)
                    and a.question.checklist_type == "site_transfer_per_site"
                    and getattr(a.apiary_site, "id", None) == getattr(site.apiary_site, "id", None)
                ]
                answers.sort(key=lambda a: (getattr(a.question, "order", 0), getattr(a, "id", 0)))
                qs = ApiaryChecklistAnswerSerializer(answers, many=True).data
                referral_list.append(
...
class CreateProposalApiarySiteTransferSerializer(serializers.ModelSerializer):
    proposal_id = serializers.IntegerField(required=True, write_only=True, allow_null=False)
    originating_approval_id = serializers.IntegerField(required=True, write_only=True, allow_null=False)
...
class UserApiaryApprovalSerializer(serializers.ModelSerializer):
    ...
    def get_licence_holders(self, obj):
        ...
        for individual_approval in obj.disturbance_proxy_approvals.filter(
            status="current", apiary_approval=True
