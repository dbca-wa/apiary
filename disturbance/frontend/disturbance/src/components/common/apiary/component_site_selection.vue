    ...
    <div class="row col-sm-12">
      ...
      <template v-if="show_view_all_features_button">
        <div class="d-flex align-items-end mt-3">
          <button class="btn btn-primary ms-auto" @click="displayAllFeatures">
...
export default {
        ...
        columns: [
          ...
          },
          {
            ...
            mRender: function (data, type, apiary_site) {
              ...
              if (apiary_site.checked) {
                return (
                  '<input type="checkbox" class="site_checkbox" data-apiary-site-id="' +
                  apiary_site.id +
                  '"' +
                  disabled_str +
                  " checked/>"
            ...
            mRender: function (data, type, apiary_site) {
              ...
              if (status_for_colour === "denied") {
                sub_str =
                  ...
                  '<line x1="4" y1="4" x2="16" y2="16" stroke="' +
                  strokeColour +
                  '" + stroke-width="2" />' +
                  '<line x1="4" y1="16" x2="16" y2="4" stroke="' +
                  strokeColour +
                  '" + stroke-width="2" />' +
                  "</svg> site: " +
                  apiary_site.id;
              ...
              return (
                '<div data-site="' + apiary_site.id + '">' + sub_str + "</div>"
              );
            },
            defaultContent: "",
          },
          ...
          },
          {
            ...
            mRender: function (data, type, apiary_site) {
              ...
              if (apiary_site.properties.licensed_site) {
                return (
                  '<input type="checkbox" class="licensed_site_checkbox" data-apiary-licensed-site-id="' +
                  apiary_site.id +
                  '"' +
                  disabled_str +
                  " checked/>"
          ...
          },
          {
            ...
            mRender: function (data, type, apiary_site) {
              let status = "pending";
              let is_vacant =
                apiary_site.properties.apiary_site_is_vacant_when_submitted;
              if (is_vacant === true) {
                status = "vacant";
              }
          ...
          },
          {
            ...
            mRender: function (data, type, apiary_site) {
              ...
              if (status == "approved") {
                ...
                return (
                  '<i class="fa fa-check" aria-hidden="true" style="color:' +
                  myColour +
                  ';"></i> Approved'
                );
                ...
                let sub_str =
                  '<i class="fa fa-times" aria-hidden="true" style="color:' +
                  myColour +
                  ';"></i> Denied';
                return sub_str;
              } else {
          ...
          },
          {
            ...
            mRender: function (data, type, apiary_site) {
              ...
              if (vm.show_action_available_unavailable) {
                ...
                if (
                  ...
                  let ret =
                    ...
                    apiary_site.id +
                    '" data-apiary-site-available="' +
                    apiary_site.properties.available +
                    '">' +
                    display_text +
                    "</a>";
              ...
              if (vm.show_action_suspend_reinstate) {
                ...
                if (status === "current") {
                  action_list.push(
                    '<a href="#" data-suspend-site="' +
                      apiary_site.id +
                      '">Suspend</a>',
                  );
                } else if (status === "suspended") {
                  action_list.push(
                    '<a href="#" data-reinstate-site="' +
                      apiary_site.id +
                      '">Reinstate</a>',
                  );
                }
  ...
  created: function () {
    ...
    if (vm.apiary_proposal_id) {
      ...
      let url_sites =
        "/api/proposal_apiary/" + vm.apiary_proposal_id + "/apiary_sites/";
      ...
      fetch(url_sites)
        .then(async (response) => {
          ...
          ((vm.apiary_sites_local = JSON.parse(
            JSON.stringify(apiary_sites_req),
          ).features), // Deep copy the array
      ...
      let url_sites =
        "/api/approvals/" + vm.apiary_approval_id + "/apiary_sites/";
      console.log("apiary_sites");
      fetch(url_sites)
        .then(async (response) => {
          if (!response.ok) {
  ...
  methods: {
    ...
    updateTableByFeatures: function (features) {
      ...
      let apiary_sites_filtered = this.apiary_sites_local.filter(
        function (apiary_site) {
      ...
      if (this.$refs.table_apiary_site) {
        ...
        if (apiary_sites.length > 0) {
          ...
          if (apiary_sites.length > 1) {
            ...
            let colOne = this.$refs.table_apiary_site.vmDataTable
              .column(1)
              .header();
    ...
    addEventListeners: function () {
      ...
      $("#" + this.table_id).on(
        "click",
        "a[data-toggle-availability]",
        this.toggleAvailability,
      );
      $("#" + this.table_id).on(
        "click",
        'input[class="site_checkbox"]',
        this.checkboxClicked,
      );
      $("#" + this.table_id).on(
        "click",
        'input[class="licensed_site_checkbox"]',
        this.checkboxLicensedSiteClicked,
      );
      $("#" + this.table_id).on(
        "click",
        'input[class="select_all_checkbox"]',
        this.checkboxSelectAll,
      );
      $("#" + this.table_id).on(
        "click",
        "a[data-make-vacant]",
        this.makeVacantClicked,
      );
      $("#" + this.table_id).on(
        "click",
        "a[data-contact-licence-holder]",
        this.contactLicenceHolder,
      );
      $("#" + this.table_id).on(
        "click",
        "a[data-suspend-site]",
        this.suspendApiarySite,
      );
      $("#" + this.table_id).on(
        "click",
        "a[data-reinstate-site]",
        this.reinstateApiarySite,
      );
    ...
    checkboxClicked: function (e) {
      ...
      this.$refs.component_map.setApiarySiteSelectedStatus(
        apiary_site_id,
        checked_status,
      );
      e.stopPropagation();
    ...
    makeVacantClicked: function (e) {
      ...
      swal
        .fire({
          title: "Make Vacant",
          text:
            "Are you sure you want to make this apiary site: " +
            apiary_site_id +
            " vacant?",
          icon: "question",
    ...
    mouseEnter: function (e) {
      ...
      if (!vm.not_close_popup_by_mouseleave) {
        let apiary_site_id = e.currentTarget.getAttribute(
          "data-apiary-site-id",
        );
        if (apiary_site_id) {
    ...
    toggleAvailability: async function (e) {
      ...
      let requested_availability =
        current_availability === "true" ? false : true;
      e.stopPropagation();

      try {
        console.log("apiary_site");
        const response = await fetch(
          "/api/apiary_site/" + apiary_site_id + "/toggle_availability/",
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
    ...
    getApiarySiteIdFromEvent(e) {
      let apiary_site_id = e.target.getAttribute("data-apiary-site-id");
      if (!apiary_site_id) {
        apiary_site_id = e.target
          .getElementsByTagName("span")[0]
          .getAttribute("data-apiary-site-id");
      }
      return apiary_site_id;
    },
    getApiaryLicensedSiteIdFromEvent(e) {
      let apiary_site_id = e.target.getAttribute(
        "data-apiary-licensed-site-id",
      );
      if (!apiary_site_id) {
        apiary_site_id = e.target
          .getElementsByTagName("span")[0]
          .getAttribute("data-apiary-licensed-site-id");
      }
      return apiary_site_id;
    },
    getApiarySiteAvailableFromEvent(e) {
      let apiary_site_available = e.target.getAttribute(
        "data-apiary-site-available",
      );

      if (!apiary_site_available) {
        apiary_site_available = e.target
          .getElementsByTagName("span")[0]
          .getAttribute("data-apiary-site-available");
      }
