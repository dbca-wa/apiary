<template lang="html">
  <div>
    <div
      class="col-sm-12"
      v-if="
        !readonly && (!is_proposal_type_renewal || proposal.is_internal_user)
      "
    >
      Mark the location of the new proposed site either by entering the latitude
      and longitude or by clicking the location in the map.
    </div>
    ...
    <template v-if="display_debug_info && proposal && proposal.proposal_apiary">
      <div class="row col-sm-12 debug-info">
        <div>
          ...
          <div>
            <div>
              Previously paid sites 'South West' region:
              {{ num_of_sites_remain_south_west }} (${{ fee_south_west }})
            </div>
            <div>Total fee: {{ total_fee_south_west }}</div>
          </div>
          <div>
            <div>
              Previously paid sites 'Remote' region:
              {{ num_of_sites_remain_remote }} (${{ fee_remote }})
            </div>
    ...
    <div class="row col-sm-12">
      <datatable
        @hook:mounted="datatable_mounted"
        ref="site_locations_table"
        id="site-locations-table"
        :dtOptions="dtOptions"
        :dtHeaders="dtHeaders"
      />
      <span class="view_all_button action_link" @click="displayAllFeatures"
        >View All Proposed Sites On Map</span
      >
    </div>
      ...
      <div id="map" class="map">
        <div :id="search_box_id" class="search-box">
          <input
            :id="search_input_id"
            class="search-input"
            placeholder="longitude, latitude OR address to search"
          />
        </div>
        ...
        >
          <g transform="scale(3)">
            <path
              ...
              style="fill: #ffffff; fill-opacity: 1; stroke-width: 0.182031"
              id="path846"
            />
            <path
    ...
    <div class="row col-sm-12" v-if="is_proposal_type_renewal">
      <label>
        Click <a href="/external/available_sites/">here</a> if you are
        interested in existing sites that are available by the site licence
        holder.
      </label>
    </div>
  </div>
...
import {
  getStatusForColour,
  getApiaryFeatureStyle,
  SiteColours,
  zoomToCoordinates,
  checkIfValidlatitudeAndlongitude,
} from "@/components/common/apiary/site_colours.js";
import Overlay from "ol/Overlay";

import MeasureStyles, {
  formatLength,
...
export default {
        ...
        columns: [
          ...
          },
          {
            ...
            mRender: function (data, type, feature) {
              ...
              if (cat) {
                cat = cat.replace("_", " ");
                return cat.replace(/\w\S*/g, function (txt) {
                  return (
          ...
          },
          {
            ...
            mRender: function (data, type, feature) {
              let action_list = [];
              let ret_str_delete =
                '<span class="delete_button action_link" data-site-location-guid="' +
                feature.getId() +
                '">Delete</span>';
              let ret_str_view =
                '<span class="view_on_map action_link" data-apiary-site-id="' +
                feature.getId() +
                '"/>View on map</span>';
              ...
              if (
                !vm.readonly &&
                (!vm.is_proposal_type_renewal || vm.proposal.is_internal_user)
              ) {
  ...
  computed: {
    ...
    num_of_sites_remain_south_west: function () {
      ...
      let value =
        this.num_of_sites_remain_south_west_base -
        this.num_of_sites_south_west_applied_unpaid;
    ...
    quotient_south_west: function () {
      return Math.floor(
        this.num_of_sites_south_west_after_deduction /
          this.min_num_of_sites_for_new,
      );
    },
    ...
    num_of_sites_south_west_to_add_as_remainder: function () {
      return (
        this.num_of_sites_south_west_calculate -
        this.num_of_sites_south_west_after_deduction
      );
    },
    total_fee_south_west: function () {
      let total_fee =
        this.num_of_sites_south_west_calculate * this.fee_south_west;
      return total_fee;
    },
    // 1.2 Renewal
    num_of_sites_remain_south_west_renewal: function () {
      ...
      let value =
        this.num_of_sites_remain_south_west_renewal_base -
    ...
    quotient_south_west_renewal: function () {
      return Math.floor(
        this.num_of_sites_south_west_renewal_after_deduction /
          this.min_num_of_sites_for_renewal,
      );
    },
    ...
    num_of_sites_south_west_renewal_calculate: function () {
      let ret_value =
        this.quotient_south_west_renewal * this.min_num_of_sites_for_renewal;
    ...
    num_of_sites_south_west_renewal_to_add_as_remainder: function () {
      return (
        this.num_of_sites_south_west_renewal_calculate -
        this.num_of_sites_south_west_renewal_after_deduction
      );
    },
    ...
    num_of_sites_remain_remote: function () {
      let value =
        this.num_of_sites_remain_remote_base -
    ...
    quotient_remote: function () {
      return Math.floor(
        this.num_of_sites_remote_after_deduction /
          this.min_num_of_sites_for_new,
      );
    },
    remainder_remote: function () {
      return (
        this.num_of_sites_remote_after_deduction % this.min_num_of_sites_for_new
      );
    },
    ...
    num_of_sites_remote_to_add_as_remainder: function () {
      return (
        this.num_of_sites_remote_calculate -
        this.num_of_sites_remote_after_deduction
      );
    },
    ...
    num_of_sites_remain_remote_renewal: function () {
      let value =
        this.num_of_sites_remain_remote_renewal_base -
    ...
    quotient_remote_renewal: function () {
      return Math.floor(
        this.num_of_sites_remote_renewal_after_deduction /
          this.min_num_of_sites_for_renewal,
      );
    },
    ...
    num_of_sites_remote_renewal_calculate: function () {
      let ret_value =
        this.quotient_remote_renewal * this.min_num_of_sites_for_renewal;
    ...
    num_of_sites_remote_renewal_to_add_as_remainder: function () {
      return (
        this.num_of_sites_remote_renewal_calculate -
        this.num_of_sites_remote_renewal_after_deduction
      );
    },
    total_fee_remote_renewal: function () {
      let total_fee =
        this.num_of_sites_remote_renewal_calculate * this.fee_remote_renewal;
      return total_fee;
    },
  ...
  watch: {
    ...
    total_num_of_sites_on_map_unpaid: function () {
      this.$emit(
        ...
        this.total_num_of_sites_on_map_unpaid,
      );
    },
    num_of_sites_south_west_to_add_as_remainder: function () {
      this.$emit(
  ...
  methods: {
        ...
        .on("awesomplete-selectcomplete", function (ev) {
          ...
          for (var i = 0; i < vm.suggest_list.length; i++) {
            if (vm.suggest_list[i].value == ev.target.value) {
              var latlng = {
                lat: vm.suggest_list[i].feature.geometry.coordinates[1],
                lng: vm.suggest_list[i].feature.geometry.coordinates[0],
              };
              zoomToCoordinates(
                vm.map,
    ...
    changeLayerVisibility: function (targetLayer) {
      ...
      for (var i = 0; i < layers.array_.length; i++) {
        if (
          layers.array_[i].get("title") === "Drawing Layer" ||
          layers.array_[i].get("title") === "Cluster Layer"
        ) {
          try {
        ...
        .then(async (response) => {
          ...
          for (let i = 0; i < layers.length; i++) {
            ...
            const wmsSource = new TileWMS({
              url: layer.layer_group_name
                ? `/kb-proxy/geoserver/${layer.layer_group_name}/wms`
    ...
    load_apiary_sites_in_this_proposal: function () {
      ...
      if (this.proposal && this.proposal.proposal_apiary) {
        let url_sites =
          "/api/proposal_apiary/" +
          this.proposal.proposal_apiary.id +
          "/apiary_sites/";
        ...
        fetch(url_sites)
          .then(async (response) => {
            if (response.ok) {
              ...
              let apiary_sites = JSON.parse(
                JSON.stringify(apiary_sites_req),
              ).features;
              for (let i = 0; i < apiary_sites.length; i++) {
                ...
                if (apiary_site.properties.status === "vacant") {
                  ...
                  let feature = vm.apiarySitesQuerySource.getFeatureById(
                    apiary_site.id,
                  );
    ...
    howPopupForLayersJson: function (
      geojson,
      coord,
      column_names,
      display_all_columns,
    ...
    metersToNearest: function (coords, filter) {
      ...
      let nearestDrawnSite =
        this.drawingLayerSource.getClosestFeatureToCoordinate(coords, filter);
      if (nearestDrawnSite != null) {
        candidates.push(
          getDistance(coords, nearestDrawnSite.getGeometry().getCoordinates()),
        );
      }

      // Retrieve the nearest apiary site from the existing apiary_sites
      let nearestQuerySite =
        this.apiarySitesQuerySource.getClosestFeatureToCoordinate(
          coords,
          filter,
        );
      if (nearestQuerySite != null) {
        candidates.push(
          getDistance(coords, nearestQuerySite.getGeometry().getCoordinates()),
        );
      }
    ...
    removeBufferForSite: function (site) {
      let buffer = this.bufferLayerSource.getFeatureById(
        site.getId() + "_buffer",
      );
      if (buffer) {
    ...
    make_remainders_reactive: function () {
      ...
      for (let i = 0; i < remainders.length; i++) {
        if (remainders[i].category_name == "South West") {
          ...
          this.num_of_sites_remain_south_west_renewal_base =
            remainders[i].remainders_renewal;
    ...
    calculateRemainders: function (features) {
      ...
      if (
        this.num_of_sites_remain_south_west >= 0 &&
        this.num_of_sites_remain_remote >= 0 &&
    ...
    constructSiteLocationsTable: function () {
      if (this.drawingLayerSource && this.$refs.site_locations_table) {
        ...
        for (let i = 0; i < features.length; i++) {
          this.$refs.site_locations_table.vmDataTable.row
            .add(features[i])
            .draw();
        }
    ...
    addEventListeners: function () {
      ...
      $("#site-locations-table").on(
        "click",
        ".delete_button",
        this.removeSiteLocation,
      );
      $("#site-locations-table").on(
        "click",
        ".view_on_map",
        this.zoomOnApiarySite,
      );
    ...
    removeSiteLocation: function (e) {
      ...
      let myFeature =
        this.drawingLayerSource.getFeatureById(site_location_guid);
      this.deleteApiarySite(myFeature);
    ...
    initMap: async function () {
      ...
      let styleCache = {};
      vm.apiarySitesClusterLayer = markRaw(
        new VectorLayer({
          title: "Cluster Layer",
      ...
      vm.map.on("moveend", function (attributes) {
        ...
        vm.apiarySitesQuerySource.forEachFeatureInExtent(
          vm.map.getView().calculateExtent(),
          function (feature) {
            // let fresh = 0;
            // let cached = 0;
        ...
        vm.drawForApiarySite.on("drawstart", async function (attributes) {
          if (vm.mode === "normal") {
            ...
            if (vm.vacant_site_being_selected) {
              ...
              vm.vacant_site_being_selected
                .getGeometry()
                .on("change", function () {
                  if (
                    modifyInProgressList.indexOf(
                      vm.vacant_site_being_selected.getId(),
                    ) == -1
                  ) {
                    modifyInProgressList.push(
                      vm.vacant_site_being_selected.getId(),
                    );
                  }
                });
      ...
      hoverInteraction.on("select", function (evt) {
        if (
          ...
          evt.selected[0].get("features").length > 0
        ) {
          // Mouse hover in
          ...
          if (is_vacant && !making_payment && status != "pending") {
            ...
            let style_applied = getApiaryFeatureStyle(
              vm.vacant_site_being_selected.get("status"),
              true,
              5,
            );
          ...
          if (vm.vacant_site_being_selected) {
            ...
            let vacant_selected =
              vm.vacant_site_being_selected.get("vacant_selected");
            if (vacant_selected) {
    ...
    keydown: function (evt) {
      ...
      if (charCode === 27 && vm.measuring === true) {
        //esc key
        //dispatch event
    ...
    display_duration: function (label) {
      ...
      if (
        this.proposal_vacant_draft_loaded &&
        ...
        console.log(
          "total time: " + timeDiff + " [ms] (" + features.length + " sites)",
        );
      }
    },
    load_existing_sites: function () {
      ...
      fetch(url)
        ...
        .then((body) => {
          let num_sites = 0;
          if (body.features) {
            vm.apiarySitesQuerySource.addFeatures(
              new GeoJSON().readFeatures(body),
            );
      ...
      fetch(
        `/api/apiary_site/list_existing_proposal_vacant_processed/?proposal_id=${this.proposal.id}`,
      )
        .then((response) => {
          if (!response.ok)
            throw new Error(`HTTP error! status: ${response.status}`);
      ...
      // approval vacant
      fetch(
        `/api/apiary_site/list_existing_approval_vacant/?proposal_id=${this.proposal.id}`,
      )
        .then((response) => {
          if (!response.ok)
            throw new Error(`HTTP error! status: ${response.status}`);
      ...
      // proposal draft
      fetch(
        `/api/apiary_site/list_existing_proposal_draft/?proposal_id=${this.proposal.id}`,
      )
        .then((response) => {
          if (!response.ok)
            throw new Error(`HTTP error! status: ${response.status}`);
      ...
      // proposal processed
      fetch(
        `/api/apiary_site/list_existing_proposal_processed/?proposal_id=${this.proposal.id}`,
      )
        .then((response) => {
          if (!response.ok)
            throw new Error(`HTTP error! status: ${response.status}`);
      ...
      // approval
      fetch(
        `/api/apiary_site/list_existing_approval/?proposal_id=${this.proposal.id}`,
      )
        .then((response) => {
          if (!response.ok)
            throw new Error(`HTTP error! status: ${response.status}`);
