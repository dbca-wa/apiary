    ...
    <div class="map-wrapper">
      ...
      <div class="basemap-button">
        <img
          ...
          :id="idBasemapSat"
          class="basemap-button-img"
          :src="layersSvgUrl"
          @click.stop="setBaseLayer('sat')"
        />
        <img
          ...
          class="basemap-button-img"
          :src="mapIconUrl"
          @click.stop="setBaseLayer('osm')"
        />
      </div>
      <div class="cursor-location">
        <div v-if="cursor_location">
          <span
            >{{ cursor_location.lat.toFixed(5) }},
            {{ cursor_location.lng.toFixed(5) }}</span
          >
        </div>
      </div>
...
Leaf.TileLayer.WMTS = Leaf.TileLayer.extend({
  ...
  getTileUrl: function (coords) {
    ...
    return (
      url +
      Leaf.Util.getParamString(this.wmtsParams, url) +
      "&tilematrix=" +
      tilematrix +
      "&tilerow=" +
      tilerow +
...
export default {
  ...
  data: function () {
    ...
    vm.icon_default = Leaf.icon({
      iconUrl: new URL("../../assets/marker-gray-locked.svg", import.meta.url)
        .href,
      ...baseDic,
    });
    vm.guid = guid();
  ...
  methods: {
    ...
    setBaseLayer: function (selected_layer_name) {
      ...
      if (selected_layer_name == "sat") {
        vm.tileLayer.setVisible(false);
        vm.tileLayerSat.setVisible(true);
        vm.currentBasemap = "sat";
      } else {
        vm.tileLayer.setVisible(true);
    ...
    initMap: function () {
      ...
      if (this.marker_lat && this.marker_lng) {
        this.mainMap = Leaf.map(this.idMap).setView(
          [this.marker_lat, this.marker_lng],
          12,
        );
      } else {
